...
class Tilemap {
    ...
    renderLayers(layers) {
        layers = jquery__WEBPACK_IMPORTED_MODULE_0___default.a.isArray(layers) ? layers : this.data.layers;
        for(var i = 0; i < layers.length; i++)
            this.renderLayer(layers[i]);
    }
    loadTileset(json) {
        this.data = json;
        this.tileset = jquery__WEBPACK_IMPORTED_MODULE_0___default()("<img />", {src: json.tilesets[0].image})[0];
        this.renderLayer();
    }
    render(deltaT, contex){
        ...
        return jquery__WEBPACK_IMPORTED_MODULE_0___default.a.ajax({
            ...
            dataType: "text json",
            async: false
        }).done(jquery__WEBPACK_IMPORTED_MODULE_0___default.a.proxy(this.loadTileset, this));
    }
    update(deltaT, input) {
